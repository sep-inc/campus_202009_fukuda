#include "PacMenMonster.h"
#include <cstdlib>
#include <ctime>
#include <cstring>

PacMenMonster::PacMenMonster()
{
	m_param.m_pos.m_x = 0;
	m_param.m_pos.m_y = 0;
	m_param.m_type = PacMenObjectType::MONSTER;
	strcpy_s(m_param.m_draw_string, PACMEN_DRAW_STRING_SIZE, " @");
...
void PacMenMonster::Init(PacMenGameMap* map_)
{
	m_p_game_map = map_;
	// 生成可能場所の取得
...
void PacMenMonster::SelectMovePos()
{
	...
	// 移動先の選択
	int select_index = rand() % array_index;
	Vec2_Int move_pos = can_move_array[select_index];
	// マップへ情報をセット
